...
Command-line credential management for nxusKit providers.
...
```bash
# List all providers and auth status
cargo run -- status

# Store an API key (interactive, masked input)
cargo run -- set openai

# Remove a stored credential
cargo run -- remove openai

# Open provider dashboard in browser
cargo run -- dashboard openai
```
